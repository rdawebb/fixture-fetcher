...
from src.utils.errors import DataProcessingError, ICSReadError
from src.utils.logging import get_logger

logger = get_logger(__name__)


def get_upcoming_events(ics_file: Path) -> Set[str]:
    ...
    try:
        ...
        raise DataProcessingError(f"Error processing events in {ics_file}: {e}") from e

    return upcoming_events


def compare_calendars(old_dir: Path, new_dir: Path) -> bool:
    ...
    for ics_file in sorted(old_dir.glob("*.ics")):
        try:
            old_signature.update(get_upcoming_events(ics_file))
    ...
    for ics_file in sorted(new_dir.glob("*.ics")):
        try:
            new_signature.update(get_upcoming_events(ics_file))
        except (ICSReadError, DataProcessingError) as e:
            logger.warning(f"Error processing {ics_file}: {e}")
